...
export default function DealNegotiationPanel({ request, theaterId, onBack }: Props) {
  const { user } = useAuth();
  ...
  // Accept a final offer from restaurant - creates the deal
  const acceptFinalOffer = async (neg: DealNegotiation) => {
    if (!request.requester_restaurant && !request.recipient_restaurant) return;

    const restaurantId = request.requester_restaurant?.id || request.recipient_restaurant?.id;
  ...
  const approveDeal = async () => {
    if (!comboDeal) return;
    await supabase.from('combo_deals').update({ theater_approved: true, is_active: true }).eq('id', comboDeal.id);
    loadData();
  };
  ...
  const canCreateDeal = !comboDeal && latestNegotiation?.is_final_offer && latestNegotiation?.sender_type === 'restaurant';
  const canApproveDeal = comboDeal && !comboDeal.theater_approved;
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400">Discuss terms with your restaurant partner</p>
      </div>
      ...
      {canCreateDeal && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-5 mb-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <p className="text-amber-300 font-bold text-lg">Final Offer from Restaurant!</p>
          ...
          <div className="bg-gray-900 rounded-xl p-4 mb-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              ...
              <div className="bg-gray-800 rounded-lg p-3">
                ...
                <p className="text-white font-medium">{latestNegotiation.proposed_commission_split.theater}%</p>
              </div>
            </div>
          </div>
      ...
      {canApproveDeal && (
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-5 mb-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-blue-300 font-bold">Deal Awaiting Your Approval</p>
          </div>
          <p className="text-blue-200/70 text-sm mb-4">The restaurant has created this deal. Approve to make it visible to customers.</p>
          <button onClick={approveDeal} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3 rounded-xl">
            Approve Deal
          </button>
        </div>
      )}
        ...
        {negotiations.length === 0 ? (
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {negotiations.map(neg => (
              <div key={neg.id} className={`rounded-xl p-4 ${neg.sender_type === 'theater' ? 'bg-blue-500/10 border border-blue-500/20' : 'bg-emerald-500/10 border border-emerald-500/20'}`}>
                <div className="flex items-center justify-between mb-3">
                  <span className={`text-xs font-medium ${neg.sender_type === 'theater' ? 'text-blue-400' : 'text-emerald-400'}`}>
                    {neg.sender_type === 'theater' ? 'Your Proposal' : 'Restaurant Proposal'}
                  </span>
      ...
      {/* Proposal Form - only show if deal not yet active */}
      {!isFullyApproved && (
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800">
          ...
          <div className="mb-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-2">
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 text-center">
                <p className="text-blue-400 text-xs mb-1">Your Share (Theater)</p>
                <p className="text-white text-xl font-bold">{form.proposed_commission_split.theater}%</p>
              </div>
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3 text-center">
                <p className="text-emerald-400 text-xs mb-1">Restaurant Share</p>
            ...
            <input
              ...
              max="100"
              step="5"
              value={form.proposed_commission_split.theater}
              onChange={e => setForm(p => ({
                ...p,
                proposed_commission_split: { theater: parseInt(e.target.value), restaurant: 100 - parseInt(e.target.value) }
              }))}
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1.5">Min Tickets Required</label>
              <select
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-white text-sm capitalize"
              >
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              placeholder="Valid for weekday shows only..."
              rows={2}
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2.5 text-white placeholder-gray-600 text-sm resize-none"
            />
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer mb-4">
            <input
              type="checkbox"
              checked={form.is_final_offer}
              onChange={e => setForm(p => ({ ...p, is_final_offer: e.target.checked }))}
          ...
          <button
            ...
            disabled={sending}
            className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-gray-800 text-white font-semibold py-3 rounded-xl flex items-center justify-center gap-2"
          >
